...
answer: the africa the most density america the most continend in
plastic waste per cap **NOTE:** From this point onwards the plots and
the output of the code are not displayed in the lab instructions, but
you can and should write the code and view the results yourself.
...
answer:box plots provid a coarser summary of the data , provide more of
summary of a distribution can also be seen as advantage in cartain
...
anwer:europe (positive) relationship , africa is the most
(mismanged\_plastic waste per cap )
...
``` r
ggplot(data = plastic_waste,
       mapping = aes(x =total_pop,color = continent, fill = continent,                y = plastic_waste_per_cap)) +
    geom_point()
```

``` r
ggplot(data = plastic_waste,
       mapping = aes(x =coastal_pop,color = continent, fill = continent,                y = plastic_waste_per_cap)) +
    geom_point()
```
